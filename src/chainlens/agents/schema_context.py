...
def plan_common_question(question: str) -> dict[str, object] | None:
    ...
    if "经营状态" in text or "企业状态" in text:
        return {
            "title": "企业经营状态分布",
            "sql": (
    ...
    if "注册资本" in text and any(word in text for word in ("区间", "分布", "结构")):
        capital_case = (
    ...
    if any(word in text for word in ("成立年份", "成立年度", "成立趋势", "新设企业")):
        return {
            "title": "企业成立年份趋势",
            "sql": (
    ...
    if "融资轮次" in text or ("融资" in text and "轮次" in text):
        return {
            "title": "融资轮次分布",
            "sql": (
    ...
    if "招投标" in text and any(word in text for word in ("年份", "年度", "趋势", "按年")):
        return {
            "title": "招投标年度趋势",
            "sql": (
    ...
    if any(word in text for word in ("地区分布", "区域分布", "区县分布")):
    ...
    if "对外投资" in text and any(word in lower for word in ("top", "最多", "排名", "前")):
    ...
    if "行业" in text and any(word in lower for word in ("top", "最多", "排名", "数量")):
        limit = _requested_limit(text)
        return {
            "title": "企业行业代码分布",
            "sql": (
